...
data    = pd.read_csv("T_unsteady.csv",header = None)
xydf    = pd.read_csv("x_y_unsteady.csv", header = None)
Tmid_t_df  = pd.read_csv("Tmid_t.csv", header = None)
#print('\nAlgandmed\n',data.head())
origin = 'lower'
Tnum = len(data)
T = data.drop(data.columns[[Tnum]], axis=1)
#print('\nAlgandmed\n',T.head())
x_feat = xydf.drop(data.columns[[1]], axis=1)
y_feat = xydf.drop(data.columns[[0]], axis=1)
#print(x_feat.head())
Tmid = Tmid_t_df.drop(data.columns[[1]], axis=1)
t    = Tmid_t_df.drop(data.columns[[0]], axis=1)
